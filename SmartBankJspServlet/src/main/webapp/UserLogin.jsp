...
<body>
...

	{
...

		{
			if(name.equals("Alan"))
			{
				RequestDispatcher req = request.getRequestDispatcher("UserLoginPage.jsp");
				req.include(request, response);
				//response.sendRedirect("admin.jsp");
			}
			else
			{
				if(rs.getString("PassChanged").equals("Yes"))
				{
					request.setAttribute("loginid",rs.getString("UserLoginId"));
				request.getRequestDispatcher("user.jsp").forward(request, response);
				
				
				}
				else if(rs.getString("PassChanged").equals("No"))
				{	
					request.setAttribute("loginid",rs.getString("UserLoginId"));
	...
	%>
		...
		<p style="color: red; margin-top: 150px; text-align: center;">Sorry, your record is not available.</p>
	<%
